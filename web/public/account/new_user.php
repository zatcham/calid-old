        ...
        if ($form_error == "") { // only continue if no errors
            if ($x = Account::createNewUser($_POST['uname'], $_POST['inp-email'], $_POST['inp-cpassword'], $_POST['role-select'])) {
                if ($x != False) {
                    ...
                    Logging::log("info", "User created successfully in account/new_user. Details: User ID: $userid, ID for user created: $x, Username for user created: $c_usr, Email for user created: $c_em ");
                    ...
                    $c_usr = $_POST['uname'];
                    $c_em = $_POST['inp-email'];
                    Logging::log("error", "Problem occured in account/new_user, Type: Issue creating user, Details: User ID: $userid, Username for user to create: $c_usr, Email for user to create: $c_em ");
                }
            } else {
                $form_error = "An unexpected error occured whilst trying to create the user";
                $c_usr = $_POST['uname'];
